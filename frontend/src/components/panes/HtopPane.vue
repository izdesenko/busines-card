...
const totalSkills = computed(() =>
  skillsStore.groups.reduce((sum, g) => sum + g.skills.length, 0),
);
  ...
  <div class="p-6 sm:p-7">
    ...
    <template v-else>
      <p class="text-xs text-faint mb-0.5">
        Tasks: {{ totalSkills }} total, {{ totalSkills }} running · Load average: {{ loadAverage }}
      </p>
      <p class="text-xs text-faint mb-4">
        Mem <span class="text-mint">[||||||||||||</span><span class="text-border">|||||]</span> занят делом
      </p>

      <div
        v-for="group in skillsStore.groups"
        :key="group.category"
        class="mb-5 last:mb-0"
      >
        <p class="text-[11px] tracking-wide text-faint mb-1.5">{{ group.category.toUpperCase() }}</p>

        <div class="grid grid-cols-[46px_1fr_64px] gap-3.5 px-2.5 pb-1.5 text-[11px] tracking-wide text-faint border-b border-borderSoft">
        ...
        >
          ...
          <span class="flex items-center gap-2 justify-self-end w-full">
            <span class="relative flex-1 h-1.5 bg-borderSoft rounded-full overflow-hidden min-w-[50px]">
              <span
